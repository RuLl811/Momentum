...
past_3 = (monthly_ret + 1).rolling(3).apply(np.prod)-1
...
def momentum(formation):
    ...
    ret_3 = past_3.loc[end_measurement]
    ret_3 = ret_3.reset_index()
    ret_3['deciles'] = pd.qcut(ret_3.iloc[:, 1], 10, labels=False, duplicates='drop')
    ganadores = ret_3[ret_3.deciles == 9]
    perdedores = ret_3[ret_3.deciles == 0]
